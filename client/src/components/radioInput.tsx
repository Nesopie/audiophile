...
const RadioInput = ({
    ...
    const handleClick = (_event: React.MouseEvent<HTMLInputElement>) => {
        ...
        radioBtns.forEach((radioBtn) => {
            if (radioBtn.checked) {
                radioBtn.parentElement?.setAttribute(
                    "style",
                    "border: 1px solid #d87c4a"
                );
            } else {
    ...
    useEffect(() => {
        const button: HTMLInputElement | null = document.querySelector(
            "#checkout > section.section-payment-details.form-section > section > div:nth-child(2) > div:nth-child(1) > input[type=radio]"
        );
        if (!button) return;
        button.checked = true;
